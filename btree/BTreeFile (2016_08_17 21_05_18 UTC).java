...
public class BTreeFile extends IndexFile implements GlobalConst {
	...
	private final static int slotsize = 4;
	...
	private KeyClass _Delete(
						...
						IOException 
						{
							...
							if(sortPage.getType()==NodeType.LEAF){
								...
								PageId nextPage; 
								BTLeafPage leafPg;
								leafPg = new BTLeafPage(page,headerPage.get_keyType());
								KeyClass deletedKey = key;
								tmpEntry= leafPg.getFirst(cRid);
							/* First the deletion of the keys when there are no sibling nodes is handled below with three scenarios
							 * 1. The search key is found in the available scan of keys 
							 * 2. The tree has nothing other than the root node.
							 * 3. The tree is Empty. 
							 */
							while((tmpEntry!=null)&&(BT.keyCompare(key,tmpEntry.key)>=0)){
								if(leafPg.delEntry(new KeyDataEntry(key,rid))){
									//delEntry() method deletes the rid if found when compared with the key.
									if(trace != null)
									{
										trace.writeBytes("TAKEFROM node" + leafPg.getCurPage()+lineSep);
										trace.flush();
									}
								PageId leafPg_no=leafPg.getCurPage();
								if((slotsize+leafPg.available_space())<=((MAX_SPACE-HFPage.DPFIXED)/2))//leaf page will be half at this point.
									{
									unpinPage(leafPg.getCurPage(),true /* = DIRTY */);
									return null;				
									}
								else if(leafPg_no.pid == headerPage.get_rootId().pid)
									// This condition evaluates if the tree has only the root node.
								{
									if(leafPg.numberOfRecords()!=0)
									{
										unpinPage(leafPg_no,true /* = DIRTY */);
										return null;
									}
									else 
									{	//This loop is to evaluates the empty condition.
										if(trace!=null)
										{
											trace.writeBytes("DEALLOCATEROOT" + leafPg_no +lineSep);
											trace.flush();
										}
										freePage(leafPg_no);
								...
								{
									/*
									 * The deletion of the key when there are siblings to the current page.
									 * Check the siblings over the left and the right of the current page, redistribute the keys if possible 
									 * after the deletion else try to merge the keys with the sibling page when space is available in the 
									 * sibling page. 
									 */
									BTIndexPage parentPg;
									parentPg = new BTIndexPage(pinPage(parentPageId),headerPage.get_keyType());
									PageId sibPgId = new PageId();
									BTLeafPage sibPg;
									int path;
									path =parentPg.getSibling(key, sibPgId);
									if(path==0)
									{
										unpinPage(leafPg.getCurPage(),true /*=DIRTY*/);
									...
									sibPg = new BTLeafPage(pinPage(sibPgId),headerPage.get_keyType());
									if(sibPg.redistribute(leafPg, parentPg, path, deletedKey))
									{	//redistribution occurs here where the keys are accommodated with the sibling page
										//when there are spaces to allocate the keys
										if(trace!=null)
										{
											trace_children(leafPg.getCurPage());
											trace_children(sibPg.getCurPage());
										}
										unpinPage(leafPg.getCurPage(),true);
										unpinPage(sibPgId,true);
										unpinPage(parentPageId,true);
										return null;
									}
									else if((sibPg.available_space() + 2*slotsize)>=((MAX_SPACE-HFPage.DPFIXED)-leafPg.available_space()))
										{
										//Merge occurs here when the redistribution is not a possible scenario 
										//map the previous child to parent node
										KeyDataEntry oldChild;
										if(path == -1)
											oldChild = leafPg.getFirst(cRid);
										else
										{
											oldChild = sibPg.getFirst(cRid);
										}
									...
									//move the entries from rightChild to leftChild
									RID startRid = new RID();
									for(tmpEntry=rightChild.getFirst(startRid);tmpEntry!=null;tmpEntry=rightChild.getFirst(startRid))
									{
										leftChild.insertRecord(tmpEntry);
										rightChild.deleteSortedRecord(startRid);
									}
									//Re-arrange the entries after the merge has been done 
									leftChild.setNextPage(rightChild.getNextPage());
									if(rightChild.getNextPage().pid!= INVALID_PAGE)	
									{
										BTLeafPage nextLeafPage = new BTLeafPage(rightChild.getNextPage(),headerPage.get_keyType());
										nextLeafPage.setPrevPage(leftChild.getCurPage());
										unpinPage(nextLeafPage.getCurPage(),true);
									}
									if(trace!=null)
									{
										trace.writeBytes("MERGE nodes"+leftChild.getCurPage() +" "+rightChild.getCurPage()+lineSep);
										trace.flush();
									}
								...
								nextPage = leafPg.getNextPage();
								unpinPage(leafPg.getCurPage());
								if(nextPage.pid == INVALID_PAGE)
									throw new RecordNotFoundException(null,"");
								leafPg = new BTLeafPage(pinPage(nextPage),headerPage.get_keyType());
								tmpEntry = leafPg.getFirst(cRid);
							}
							//The key is not found in the initial search hence unpin the current page
							unpinPage(leafPg.getCurPage());
							throw new RecordNotFoundException(null,"");
						}
							...
							{	 
								...
								KeyClass deletedKey = key;
								RID cRid = indexPage.deleteKey(oldChildKey);
								if(indexPage.getCurPage().pid == headerPage.get_rootId().pid)
								{
									//Check the current index page is the root node
									if(indexPage.numberOfRecords()==0)
									{
										...
										if(trace !=null)
										{
											trace.writeBytes("CHANGEROOT from node" + indexPage.getCurPage()+ "to node" + indexPage.getPrevPage()+lineSep);
											trace.flush();
										}
								...
								if(( slotsize + indexPage.available_space())<=((MAX_SPACE-HFPage.DPFIXED)/2))
								{
									//(MAX_SPACE-HFPage.DPFIXED)/ 2 determines the index page is half full after deletion
								...
								else
								{
									...
									sibPg = new BTIndexPage(pinPage(sibPgId),headerPage.get_keyType());
									int pKeySize = 0;
									if(path == -1)
									{
										pKeySize = BT.getKeyLength(parentPg.findKey(indexPage.getFirst(new RID()).key));
									}
									else if(path == 0)
									{
										pKeySize = BT.getKeyLength(parentPg.findKey(sibPg.getFirst(new RID()).key));
									}
									if(sibPg.redistribute(indexPage, parentPg, path, deletedKey))
									{	//redistribution occurs here where the keys are accommodated with the sibling page
										//when there are spaces to allocate the keys
											if(trace != null)
											{
									...
									else if(sibPg.available_space()+slotsize >= ((MAX_SPACE-HFPage.DPFIXED)-(indexPage.available_space()+slotsize)+pKeySize+slotsize+4))
											{
												//Merge occurs here when the redistribution is not a possible scenario 
												//map the previous child to parent node
												KeyClass oldChild;
												if(path ==-1)
												{
													oldChild = indexPage.getFirst(cRid).key;
												}
												else 
												{
													oldChild = sibPg.getFirst(cRid).key;
												}
												...
												RID startRid = new RID();
												leftChild.insertKey(parentPg.findKey(oldChild), rightChild.getLeftLink());
												//Re-arrange the entries after the merge has been done 
												for(KeyDataEntry tmpEntry = rightChild.getFirst(startRid);tmpEntry!=null;tmpEntry=rightChild.getFirst(startRid))
												{
